#include <interception_macros.h>

namespace app::methods::System::Threading::Tasks::TaskCompletionSource_1_System::Byte___ {
IL2CPP_REGISTER_METHOD(0x02776D20, void, __ctor, (TaskCompletionSource_1_System_Byte_ * __this, Object * state));
IL2CPP_REGISTER_METHODINFO(0x04767D48, TaskCompletionSource_1_System_Byte___ctor__MethodInfo);
IL2CPP_REGISTER_METHOD(0x002FA280, Task_1_System_Byte_ *, get_Task, (TaskCompletionSource_1_System_Byte_ * __this));
IL2CPP_REGISTER_METHODINFO(0x0470E048, TaskCompletionSource_1_System_Byte__get_Task__MethodInfo);
IL2CPP_REGISTER_METHOD(0x02776BF0, void, __ctor, (TaskCompletionSource_1_System_Byte_ * __this));
IL2CPP_REGISTER_METHODINFO(0x04746440, TaskCompletionSource_1_System_Byte___ctor_1__MethodInfo);
IL2CPP_REGISTER_METHOD(0x027774F0, bool, TrySetException, (TaskCompletionSource_1_System_Byte_ * __this, Exception * exception));
IL2CPP_REGISTER_METHODINFO(0x04729098, TaskCompletionSource_1_System_Byte__TrySetException__MethodInfo);
IL2CPP_REGISTER_METHOD(0x027771C0, bool, TrySetResult, (TaskCompletionSource_1_System_Byte_ * __this, Byte__Array * result));
IL2CPP_REGISTER_METHODINFO(0x0475CA48, TaskCompletionSource_1_System_Byte__TrySetResult__MethodInfo);
IL2CPP_REGISTER_METHOD(0x02777610, void, SetException, (TaskCompletionSource_1_System_Byte_ * __this, Exception * exception));
IL2CPP_REGISTER_METHODINFO(0x04798A18, TaskCompletionSource_1_System_Byte__SetException__MethodInfo);
IL2CPP_REGISTER_METHOD(0x02777740, void, SetResult, (TaskCompletionSource_1_System_Byte_ * __this, Byte__Array * result));
IL2CPP_REGISTER_METHODINFO(0x04726560, TaskCompletionSource_1_System_Byte__SetResult__MethodInfo);
}
