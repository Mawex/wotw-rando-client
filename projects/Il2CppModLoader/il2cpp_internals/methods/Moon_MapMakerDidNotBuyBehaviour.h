#include <interception_macros.h>

namespace app::methods::Moon::MapMakerDidNotBuyBehaviour {
IL2CPP_REGISTER_METHOD(0x0110E520, void, OnEnter, (app::MapMakerDidNotBuyBehaviour * this_ptr, app::IContext * context));
IL2CPP_REGISTER_METHOD(0x0110E610, BehaviourStatus__Enum, OnExecute, (app::MapMakerDidNotBuyBehaviour * this_ptr, app::IContext * context));
IL2CPP_REGISTER_METHOD(0x0110E640, void, OnExit, (app::MapMakerDidNotBuyBehaviour * this_ptr, app::IContext * context));
IL2CPP_REGISTER_METHOD(0x0060C1C0, void, __ctor, (app::MapMakerDidNotBuyBehaviour * this_ptr));
}
