#include <interception_macros.h>

namespace app::methods::System_Collections_Generic::List_1_SpellInventory_Binding_ {
IL2CPP_REGISTER_METHOD(0x025E8100, void, __ctor, (app::List_1_SpellInventory_Binding_ * this_ptr));
IL2CPP_REGISTER_METHODINFO(0x04721B60, List_1_SpellInventory_Binding___ctor__MethodInfo);
IL2CPP_REGISTER_METHOD(0x02F1C970, void, Add, (app::List_1_SpellInventory_Binding_ * this_ptr, app::SpellInventory_Binding__Enum item));
IL2CPP_REGISTER_METHODINFO(0x047840A8, List_1_SpellInventory_Binding__Add__MethodInfo);
IL2CPP_REGISTER_METHOD(0x02D1F360, SpellInventory_Binding__Enum, get_Item, (app::List_1_SpellInventory_Binding_ * this_ptr, int32_t index));
IL2CPP_REGISTER_METHODINFO(0x0471C070, List_1_SpellInventory_Binding__get_Item__MethodInfo);
IL2CPP_REGISTER_METHOD(0x00529FF0, int32_t, get_Count, (app::List_1_SpellInventory_Binding_ * this_ptr));
IL2CPP_REGISTER_METHODINFO(0x04725C70, List_1_SpellInventory_Binding__get_Count__MethodInfo);
IL2CPP_REGISTER_METHOD(0x022A4010, void, Clear, (app::List_1_SpellInventory_Binding_ * this_ptr));
IL2CPP_REGISTER_METHODINFO(0x047680E8, List_1_SpellInventory_Binding__Clear__MethodInfo);
}
