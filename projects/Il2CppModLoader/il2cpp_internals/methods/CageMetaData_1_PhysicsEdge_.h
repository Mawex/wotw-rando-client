#include <interception_macros.h>

namespace app::methods::CageMetaData_1_PhysicsEdge_ {
IL2CPP_REGISTER_METHOD(0x019DD450, void, Add, (CageMetaData_1_PhysicsEdge_ * __this, PhysicsEdge * dataEntry, int32_t id));
IL2CPP_REGISTER_METHODINFO(0x0477E070, CageMetaData_1_PhysicsEdge__Add__MethodInfo);
IL2CPP_REGISTER_METHOD(0x019DD7B0, PhysicsEdge *, FindByID, (CageMetaData_1_PhysicsEdge_ * __this, int32_t id));
IL2CPP_REGISTER_METHODINFO(0x0471BB00, CageMetaData_1_PhysicsEdge__FindByID__MethodInfo);
IL2CPP_REGISTER_METHOD(0x019DD620, void, Remove, (CageMetaData_1_PhysicsEdge_ * __this, int32_t id));
IL2CPP_REGISTER_METHODINFO(0x047554C0, CageMetaData_1_PhysicsEdge__Remove__MethodInfo);
}
