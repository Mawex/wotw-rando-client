#include <interception_macros.h>

namespace app::methods::System::Threading::Tasks::Task_1_SystemIntegration::Races::RaceRequestResult_ {
IL2CPP_REGISTER_METHOD(0x02A5BF40, Task *, ContinueWith, (Task_1_SystemIntegration_Races_RaceRequestResult_ * __this, Action_1_System_Threading_Tasks_Task_1__4 * continuationAction));
IL2CPP_REGISTER_METHODINFO(0x04716A78, Task_1_SystemIntegration_Races_RaceRequestResult__ContinueWith__MethodInfo);
IL2CPP_REGISTER_METHOD(0x02A5A760, RaceRequestResult *, get_Result, (Task_1_SystemIntegration_Races_RaceRequestResult_ * __this));
IL2CPP_REGISTER_METHODINFO(0x0474C920, Task_1_SystemIntegration_Races_RaceRequestResult__get_Result__MethodInfo);
IL2CPP_REGISTER_METHOD(0x004C50A0, TaskAwaiter_1_SystemIntegration_Races_RaceRequestResult_, GetAwaiter, (Task_1_SystemIntegration_Races_RaceRequestResult_ * __this));
IL2CPP_REGISTER_METHODINFO(0x0474BD28, Task_1_SystemIntegration_Races_RaceRequestResult__GetAwaiter__MethodInfo);
}
