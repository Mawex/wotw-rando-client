using namespace app;

namespace app::methods::PhysicalSystemStateModifier {
IL2CPP_REGISTER_METHOD(0x014FBB70, void, Apply, (PhysicalSystemStateModifier * __this, MoonReference_1_UnityEngine_GameObject_ * target, UberStateApplyContext__Enum context));
IL2CPP_REGISTER_METHOD(0x014FBD90, void, __ctor, (PhysicalSystemStateModifier * __this));
IL2CPP_REGISTER_METHOD(0x014FBDB0, Dictionary_2_System_Int32_System_Type_ *, GetIdToTypeMap, (PhysicalSystemStateModifier * __this));
}
