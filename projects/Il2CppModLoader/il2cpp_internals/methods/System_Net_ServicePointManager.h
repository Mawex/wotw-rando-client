using namespace app;

namespace app::methods::System::Net::ServicePointManager {
IL2CPP_REGISTER_METHOD(0x01BD77A0, void, __cctor, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x002FA000, void, __ctor, (ServicePointManager * __this));
IL2CPP_REGISTER_METHOD(0x01BD7970, ICertificatePolicy *, get_CertificatePolicy, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD7B50, void, set_CertificatePolicy, (ICertificatePolicy * value));
IL2CPP_REGISTER_METHOD(0x01BD7C00, ICertificatePolicy *, GetLegacyCertificatePolicy, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD7CA0, bool, get_CheckCertificateRevocationList, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD7D40, void, set_CheckCertificateRevocationList, (bool value));
IL2CPP_REGISTER_METHOD(0x01BD7DE0, int32_t, get_DefaultConnectionLimit, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD7E80, void, set_DefaultConnectionLimit, (int32_t value));
IL2CPP_REGISTER_METHODINFO(0x0476D5E0, ServicePointManager_set_DefaultConnectionLimit__MethodInfo);
IL2CPP_REGISTER_METHOD(0x01BD7F60, Exception *, GetMustImplement, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD80A0, int32_t, get_DnsRefreshTimeout, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8140, void, set_DnsRefreshTimeout, (int32_t value));
IL2CPP_REGISTER_METHOD(0x01BD8230, bool, get_EnableDnsRoundRobin, (MethodInfo * method));
IL2CPP_REGISTER_METHODINFO(0x0470D4F8, ServicePointManager_get_EnableDnsRoundRobin__MethodInfo);
IL2CPP_REGISTER_METHOD(0x01BD82D0, void, set_EnableDnsRoundRobin, (bool value));
IL2CPP_REGISTER_METHODINFO(0x0472AE08, ServicePointManager_set_EnableDnsRoundRobin__MethodInfo);
IL2CPP_REGISTER_METHOD(0x01BD8370, int32_t, get_MaxServicePointIdleTime, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8410, void, set_MaxServicePointIdleTime, (int32_t value));
IL2CPP_REGISTER_METHODINFO(0x04778998, ServicePointManager_set_MaxServicePointIdleTime__MethodInfo);
IL2CPP_REGISTER_METHOD(0x01BD84F0, int32_t, get_MaxServicePoints, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8590, void, set_MaxServicePoints, (int32_t value));
IL2CPP_REGISTER_METHODINFO(0x047860E0, ServicePointManager_set_MaxServicePoints__MethodInfo);
IL2CPP_REGISTER_METHOD(0x00417870, bool, get_ReusePort, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8670, void, set_ReusePort, (bool value));
IL2CPP_REGISTER_METHODINFO(0x04776868, ServicePointManager_set_ReusePort__MethodInfo);
IL2CPP_REGISTER_METHOD(0x01BD86C0, SecurityProtocolType__Enum, get_SecurityProtocol, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8760, void, set_SecurityProtocol, (SecurityProtocolType__Enum value));
IL2CPP_REGISTER_METHOD(0x01BD8810, ServerCertValidationCallback *, get_ServerCertValidationCallback, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD88B0, RemoteCertificateValidationCallback *, get_ServerCertificateValidationCallback, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8990, void, set_ServerCertificateValidationCallback, (RemoteCertificateValidationCallback * value));
IL2CPP_REGISTER_METHOD(0x00420EE0, EncryptionPolicy__Enum, get_EncryptionPolicy, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8B60, bool, get_Expect100Continue, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8C00, void, set_Expect100Continue, (bool value));
IL2CPP_REGISTER_METHOD(0x01BD8CB0, bool, get_UseNagleAlgorithm, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8D50, void, set_UseNagleAlgorithm, (bool value));
IL2CPP_REGISTER_METHOD(0x00417870, bool, get_DisableStrongCrypto, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x00417870, bool, get_DisableSendAuxRecord, (MethodInfo * method));
IL2CPP_REGISTER_METHOD(0x01BD8E00, void, SetTcpKeepAlive, (bool enabled, int32_t keepAliveTime, int32_t keepAliveInterval));
IL2CPP_REGISTER_METHODINFO(0x0476A190, ServicePointManager_SetTcpKeepAlive__MethodInfo);
IL2CPP_REGISTER_METHOD(0x01BD8F60, ServicePoint *, FindServicePoint, (Uri * address));
IL2CPP_REGISTER_METHOD(0x01BD9000, ServicePoint *, FindServicePoint, (String * uriString, IWebProxy * proxy));
IL2CPP_REGISTER_METHOD(0x01BD9180, ServicePoint *, FindServicePoint, (Uri * address, IWebProxy * proxy));
IL2CPP_REGISTER_METHODINFO(0x04751A98, ServicePointManager_FindServicePoint_2__MethodInfo);
IL2CPP_REGISTER_METHOD(0x01BD9C30, void, CloseConnectionGroup, (String * connectionGroupName));
}
