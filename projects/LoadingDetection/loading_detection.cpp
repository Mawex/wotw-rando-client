...
namespace loading_detection {
    /** Make loading from QTMs pause the timer entirely instead only for the loading time */
    constexpr bool USE_LEGACY_QTM_TIMING = true;
    ...
    namespace {
        LoadingState detect_current_loading_state() {
            ...
            if (instant_load_scenes_controller->fields.m_isLoading) {
                ...
                if (instant_load_scenes_controller->fields.m_lockFinishLoading) {
                    ...
                    if (USE_LEGACY_QTM_TIMING) {
                        return LoadingState::LegacyQTM;
                    }
            ...
            auto scenes_manager = helpers::get_scenes_manager();

            if (scenes_manager == nullptr) {
                return LoadingState::ScenesManagerNonexistent;
            }

            auto game_controller = helpers::get_game_controller();
            ...
            if (!ignore_position_inside_scene_still_loading) {
                auto position = helpers::get_player_position();
        ...
        // Unityplayer Update loop intercept to update the loading state cache
        namespace {
